#!/usr/bin/env python3
"""
Create Stamford Heat Vulnerability Index map with original tract shapes but no overlaps
...
def create_stamford_no_overlap_original_shapes():
    """Create Stamford map with original shapes but no overlaps"""
    
    print("Creating Stamford Heat Vulnerability Index map with original shapes (no overlaps)...")
...
def create_interactive_map(stamford_gdf):
    ...
    for _, row in stamford_gdf.iterrows():
    ...
    # Save the map
    map_path = 'hvi_output/stamford_heat_vulnerability_original_shapes_no_overlap.html'
    m.save(map_path)
...
if __name__ == "__main__":
    map_obj = create_stamford_no_overlap_original_shapes()
    if map_obj:
        print("✓ Successfully created Stamford Heat Vulnerability Index map with original shapes but no overlaps")
