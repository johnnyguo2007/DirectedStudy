#!/usr/bin/env python3
"""
Create Stamford Heat Vulnerability Index map with non-overlapping regions using Voronoi diagrams
Generates the same output as hvi_output/stamford_heat_vulnerability_interactive_map.html
"""
...
def create_non_overlapping_stamford_map():
    """Create Stamford Heat Vulnerability Index map with guaranteed non-overlapping regions"""
    
    print("Creating Stamford Heat Vulnerability Index map with non-overlapping regions...")
    ...
    stamford_gdf = create_voronoi_tracts(stamford_data)
...
def create_voronoi_tracts(stamford_data):
    """Create non-overlapping census tract geometries using Voronoi diagrams"""
    ...
    # Create realistic bounds around Stamford (approximately 12km radius)
    bounds_km = 12
    ...
    # Define cluster centers based on Stamford geography
    cluster_centers = [
        (stamford_center_lat, stamford_center_lon),                    # Downtown Stamford
        (stamford_center_lat + 0.02, stamford_center_lon - 0.02),     # North Stamford
        (stamford_center_lat - 0.015, stamford_center_lon + 0.025),   # East Side
        (stamford_center_lat + 0.01, stamford_center_lon + 0.03),     # Darien border area
        (stamford_center_lat - 0.025, stamford_center_lon - 0.01),    # West Side
        (stamford_center_lat + 0.03, stamford_center_lon),            # New Canaan border
        (stamford_center_lat - 0.01, stamford_center_lon - 0.035),    # Greenwich border
    ]
    ...
    print(f"✓ Created {len(stamford_gdf)} non-overlapping tract boundaries")
    return stamford_gdf
...
def create_interactive_vulnerability_map(stamford_gdf):
    ...
    map_path = 'hvi_output/stamford_heat_vulnerability_interactive_map_non_overlapping.html'
    m.save(map_path)
...
if __name__ == "__main__":
    map_obj = create_non_overlapping_stamford_map()
    if map_obj:
        print("✓ Successfully created Stamford Heat Vulnerability Index map with non-overlapping regions")
