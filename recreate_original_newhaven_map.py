#!/usr/bin/env python3
"""
Recreate the exact New Haven Heat Vulnerability Interactive Map
Generates identical output to hvi_output/newhaven_heat_vulnerability_interactive_map.html
"""
...
def recreate_original_newhaven_map():
    """Recreate the exact original New Haven Heat Vulnerability Index map"""
    
    print("Recreating the exact original New Haven Heat Vulnerability Index map...")
    
    # Load the extracted census tract data
    try:
        with open('newhaven_census_tract_data.json', 'r') as f:
            data = json.load(f)
    ...
    map_path = 'hvi_output/newhaven_heat_vulnerability_interactive_map_recreated.html'
    m.save(map_path)
    print(f"✓ Saved recreated map to {map_path}")
    
    # Also save as exact original filename for replacement
    original_path = 'hvi_output/newhaven_heat_vulnerability_interactive_map.html'
...
if __name__ == "__main__":
    map_obj = recreate_original_newhaven_map()
    if map_obj:
        print("✓ Successfully recreated the exact original New Haven Heat Vulnerability Index map")
        print("  The map contains all census tracts with identical styling and functionality")
    else:
