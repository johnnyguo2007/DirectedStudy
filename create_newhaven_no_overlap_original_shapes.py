#!/usr/bin/env python3
"""
Create New Haven Heat Vulnerability Index map with original tract shapes but no overlaps
...
def create_newhaven_no_overlap_original_shapes():
    """Create New Haven map with original shapes but no overlaps"""
    
    print("Creating New Haven Heat Vulnerability Index map with original shapes (no overlaps)...")
...
def create_interactive_map(newhaven_gdf):
    ...
    for _, row in newhaven_gdf.iterrows():
    ...
    # Save the map
    map_path = 'hvi_output/newhaven_heat_vulnerability_original_shapes_no_overlap.html'
    m.save(map_path)
...
if __name__ == "__main__":
    map_obj = create_newhaven_no_overlap_original_shapes()
    if map_obj:
        print("✓ Successfully created New Haven Heat Vulnerability Index map with original shapes but no overlaps")
